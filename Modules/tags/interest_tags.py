...
def get_tags(likes):
    ...
    for i in interest:
        ...
        for doc in cursor:
            if doc['score'] >1.0:
                suggested_communities.extend(doc['tags'])

    suggested_communities = Counter(suggested_communities).most_common(24)
...
def search_tags(name):
    ...
    for doc in cursor:
        searched_communities.extend(doc['tags'])
    searched_communities = Counter(searched_communities).most_common(10)
